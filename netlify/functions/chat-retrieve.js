...
// netlify/functions/chat-retrieve.ts
var chat_retrieve_exports = {};
__export(chat_retrieve_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(chat_retrieve_exports);
...
function initializeResources() {
  ...
  const kbPaths = [
    (0, import_path.join)(process.cwd(), "knowledge", "kb.entries.json"),
    (0, import_path.join)(process.cwd(), "..", "knowledge", "kb.entries.json"),
    (0, import_path.join)(__dirname, "..", "..", "knowledge", "kb.entries.json")
  ];
  let foundKb = false;
  for (const path of kbPaths) {
    if ((0, import_fs.existsSync)(path)) {
      kb = JSON.parse((0, import_fs.readFileSync)(path, "utf-8"));
      foundKb = true;
      break;
    }
  }
  ...
  const cachePaths = [
    (0, import_path.join)(process.cwd(), "knowledge", ".embedding-cache.json"),
    (0, import_path.join)(process.cwd(), "..", "knowledge", ".embedding-cache.json"),
    (0, import_path.join)(__dirname, "..", "..", "knowledge", ".embedding-cache.json")
  ];
  for (const path of cachePaths) {
    if ((0, import_fs.existsSync)(path)) {
      embeddingCache = JSON.parse((0, import_fs.readFileSync)(path, "utf-8"));
      break;
    }
  }
...
function calculateSimilarity(queryEmbedding, entryId, entryTexts) {
  let maxSimilarity = 0;
