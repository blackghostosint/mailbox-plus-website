...
describe('CORS Middleware Utility', () => {
  describe('withCors (Web Standard Handler)', () => {
    ...
    it('matches allowed origin patterns dynamically against request Origin header', async () => {
      ...
      const reqPreview12 = new Request('https://example.com/api/test', {
        method: 'POST',
        headers: { origin: 'https://deploy-preview-12--mailboxplus.netlify.app' },
      });
      const resPreview12 = await wrapped(reqPreview12, {});
      expect(resPreview12.headers.get('Access-Control-Allow-Origin')).toBe(
        'https://deploy-preview-12--mailboxplus.netlify.app'
      );

      // Official Netlify production subdomains
