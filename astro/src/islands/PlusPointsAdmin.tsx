...
import type { CustomerType as Customer } from '../../../netlify/functions/customer';
import type { TransactionType as Transaction } from '../../../netlify/functions/transact';
...
export const PlusPointsAdmin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let points = floored * (selectedCustomer.multiplier ?? 1);
    if (isWednesday) {
      points *= 2;
    }
  ...
  const handleRedeem = async (rewardName: string, cost: number) => {
    ...
    if ((selectedCustomer.points ?? 0) < cost) {
      alert(`Customer has insufficient points balance (${selectedCustomer.points ?? 0} pts)`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/customer/${selectedCustomer.id}`, {
        ...
        body: JSON.stringify({
          ...
          ytdPoints: Math.max(selectedCustomer.ytdPoints ?? 0, targetYtd),
        }),
      });
                        ...
                        <div className="bg-white rounded-2xl p-6 border border-border-strong shadow-sm space-y-4">
                          ...
                          <div className="space-y-3">
                            {[
                              ...
                              <button
                                key={i}
                                disabled={(selectedCustomer.points ?? 0) < reward.cost}
                                ...
                                className={`w-full p-3.5 rounded-xl border flex justify-between items-center transition-all ${
                                  (selectedCustomer.points ?? 0) >= reward.cost
