...
async function main() {
    ...
    for (const testCase of retrievalTests) {
        ...
        console.log(`${icon} ${testCase.id}: ${testCase.query.substring(0, 50)}${testCase.query.length > 50 ? '...' : ''}${confStr}`);
    }
    ...
    console.log(`   Direct Match:    ${categoryResults.direct_match.passed}/${categoryResults.direct_match.passed + categoryResults.direct_match.failed} passed`);
    console.log(`   Paraphrase:      ${categoryResults.paraphrase.passed}/${categoryResults.paraphrase.passed + categoryResults.paraphrase.failed} passed`);
    console.log(`   Ambiguous:       ${categoryResults.ambiguous.passed}/${categoryResults.ambiguous.passed + categoryResults.ambiguous.failed} passed`);
    console.log(`   Operational:     ${categoryResults.operational.passed}/${categoryResults.operational.passed + categoryResults.operational.failed} passed`);
    console.log(`   Out-of-Scope:    ${categoryResults.out_of_scope.passed}/${categoryResults.out_of_scope.passed + categoryResults.out_of_scope.failed} passed\n`);
