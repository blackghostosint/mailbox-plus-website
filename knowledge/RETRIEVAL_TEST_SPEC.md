...
> - **Local Bootstrap Path:** When running locally on a fresh checkout without `GEMINI_API_KEY` and without a restored `.embedding-cache.json`, `npm run test:retrieval` will exit cleanly with code 0 and log a notice explaining how to run or generate embeddings. To bootstrap or refresh local vector embeddings, run `GEMINI_API_KEY=your_key npm run test:retrieval`.
