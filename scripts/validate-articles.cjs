    ...
    try {
        ...
        if (Array.isArray(data.relatedServices)) {
            data.relatedServices.forEach(servicePath => {
                ...
                if (!validRoutes.has(servicePath)) {
                    errors.push(`❌ ${path.basename(filePath)}: relatedServices path '${servicePath}' does not match any known route`);
                }
            });
        }
        ...
        while ((match = linkRegex.exec(content)) !== null) {
            ...
            if (!validRoutes.has(linkPath)) {
                errors.push(`❌ ${path.basename(filePath)}: Markdown link '${linkPath}' does not match any known route`);
            }
        }
