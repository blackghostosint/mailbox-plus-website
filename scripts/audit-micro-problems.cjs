...
function parseSearchConsoleCSV(filePath) {
    ...
    const ctrIdx = header.findIndex(h => h.toLowerCase().includes('ctr') || h.toLowerCase().includes('click-through'));
    const positionIdx = header.findIndex(h => h.toLowerCase().includes('position') || h.toLowerCase().includes('rank'));
...
function evaluateSignalB(service, staffFeedback) {
    ...
    const hasNegativeFeedback = feedback.feedback === 'negative' || 
...
function createMockCSV() {
    ...
    const mockPages = [
        ...
        ['https://mailboxplus.com/ship-a-fragile-item-safely-in-concord-township-glass-ceramics-electronics', 45, 600, 0.075, 8.5],
        ['https://mailboxplus.com/ship-an-odd-shaped-or-oversized-item-in-concord-township-guitars-lamps-sports-equipment', 0, 0, 0, 0],
    ];
