...
if (import.meta.env.DEV) {
    ...
    microProblems.forEach((mp, index) => {
        ...
        if (!mp.slug) {
            throw new Error(`Micro-problem at index ${index} (id: ${mp.id}) is missing required field: slug`);
        }
        if (!mp.serviceName) {
            throw new Error(`Micro-problem at index ${index} (id: ${mp.id}) is missing required field: serviceName`);
        }
        if (!mp.heroTitle) {
            throw new Error(`Micro-problem at index ${index} (id: ${mp.id}) is missing required field: heroTitle`);
        }
