...
// Astro island variant — accepts slug as prop instead of useParams
export const ArticlePageContent: React.FC<{ slug: string }> = ({ slug }) => {
    const [article, setArticle] = useState<Article | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(false);
    ...
    useEffect(() => {
        const loadArticle = async () => {
            try {
                setLoading(true);
                const data = await articleLoader.getArticleBySlug(slug);
                if (data) { setArticle(data); } else { setError(true); }
            } catch (err) {
                console.error("Failed to load article:", err);
                setError(true);
            } finally {
    ...
    return (
        <>
            <div className="relative bg-slate-900 py-24 lg:py-32 overflow-hidden">
                <div className="absolute inset-0 w-full h-full">
                    <SmartImage src={getServiceImageUrl(image)} alt={imageAlt || title} className="w-full h-full object-cover opacity-20 blur-sm" />
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/80 to-transparent"></div>
                </div>
                <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-200 text-sm font-medium mb-6 backdrop-blur-sm border border-blue-500/30">
                    ...
                    <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight mb-6 leading-tight">{title}</h1>
                    <p className="text-xl text-slate-300 max-w-2xl mx-auto mb-8 leading-relaxed">{description}</p>
                    <div className="flex items-center justify-center gap-6 text-slate-400 text-sm">
                        <div className="flex items-center gap-2"><Calendar className="w-4 h-4" /><span>{formattedDate}</span></div>
                        {author && <div className="flex items-center gap-2"><User className="w-4 h-4" /><span>By {author}</span></div>}
                    </div>
                </div>
            </div>
            <div className="bg-white py-16 lg:py-24">
                <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mt-20 p-8 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl shadow-xl text-center text-white">
                        <h3 className="text-2xl font-bold mb-4">Have Questions?</h3>
                        <p className="text-blue-100 mb-8 max-w-xl mx-auto">Stop by our location in Concord Township or give us a call. We&apos;re here to help with all your shipping and printing needs.</p>
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            <a href="/contact-us"><Button className="bg-white text-blue-700 hover:bg-blue-50 w-full sm:w-auto font-semibold border-none">Contact Us</Button></a>
                            <a href="/services"><Button className="bg-transparent border border-blue-400/50 text-white hover:bg-blue-600/50 w-full sm:w-auto">View All Services</Button></a>
                        </div>
                    </div>
                </div>
            </div>
