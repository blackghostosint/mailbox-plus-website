      ...
      <div className="min-h-screen bg-[var(--color-bg-primary)]">
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-[var(--color-primary-dark)] via-[var(--color-primary)] to-[var(--color-primary-deep)] py-16 lg:py-24 text-center">
          <div className="relative z-10 max-w-4xl mx-auto px-6">
            {/* Rating pill */}
            {aggregateRating && (
              <div className="inline-flex items-center gap-3 rounded-full bg-white/10 backdrop-blur-xl border border-white/40 px-4 py-1.5 mb-4 shadow-sm animate-fade-in">
            ...
            {/* Title + subtitle */}
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-white mb-6 animate-fade-in-up">
              {heroTitle}
            </h1>

            <p className="text-xl md:text-2xl text-white/80 leading-relaxed font-medium max-w-3xl mx-auto mb-10 animate-fade-in-up [animation-delay:150ms]">
              {heroSubtitle}
            </p>
        ...
        <main className="relative -mt-20 pb-20 lg:pb-28">
          <div className="container mx-auto px-4 space-y-20">
            {/* -------- GLASS INTRO CARD -------- */}
            {children && (
              <section className="max-w-3xl mx-auto reveal-on-scroll opacity-0">
            ...
            {features && features.length > 0 && (
              <section className="max-w-6xl mx-auto reveal-on-scroll opacity-0">
                ...
                <div className="grid gap-6 md:grid-cols-3">
                  {features.map((feature, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="group relative overflow-hidden rounded-2xl border border-white/70 bg-white/70 backdrop-blur-xl shadow-sm hover:-translate-y-1.5 transition-transform duration-200"
                      >
            ...
            {content &&
              content.map((block, idx) => (
                <section key={idx} className="max-w-5xl mx-auto reveal-on-scroll opacity-0">
                  <div
                    className={[
            ...
            {/* -------- FAQ (GLASS BAND) -------- */}
            {faqs && faqs.length > 0 && (
              <section className="py-12 rounded-2xl bg-gradient-to-b from-[var(--color-bg-secondary)]/80 via-[var(--color-border)]/70 to-[var(--color-bg-secondary)]/80 reveal-on-scroll opacity-0">
            ...
            {/* -------- CTA (GLASS + GRADIENT) -------- */}
            {cta && (
              <section className="max-w-4xl mx-auto reveal-on-scroll opacity-0">
